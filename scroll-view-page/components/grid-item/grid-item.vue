<template>
	<view :class="['grid-item','col-'+Col, 'order-'+Order, Border]" :hover-class="hoverClass" 
	:hover-start-time="200" @tap="onClick" :data-opentype="type" :data-url="url">
		<view class="sub">
			<view v-if="dot && badge==false" class="dot"></view>
			<view v-if="badge" class="badge">{{badge}}</view>
			
			<view :class="iconFont!=''?['o-icon-' + iconFont]:iconImg!=''?'o-img':''" >
				<image v-if="iconImg!=''" :src="iconImg" mode="aspectFit"></image>
			</view>
			
			<view class="o-text">{{text}}</view>
		</view>
	</view>
</template>

<script>
	export default {
		name:'grid-item',
		props:{
			text:{type:String,default:''},//文字内容
			iconImg:{type:String,default:''},//图片图标
			iconFont:{type:String,default:''},//字体图标
			type:{type:String,default:''},//跳转tabbar页时用
			url:{type:String,default:''},//链接地址
			hoverClass:{type:String,default:''},//按下时的样式类
			order:{type:[Number,String],default:''},//宫格排序,不支持小程序
			dot:{type:Boolean,default:false},//右上角红点
			badge:{type:[Number,String],default:''},//图标右上角徽标的内容
		},
		inject: ['col','border'],
		methods: {
			onClick(e) {
				// let url = e.currentTarget.dataset.url;
				// let type = e.currentTarget.dataset.opentype;
				// if (type=='tabbar'){
				// 	uni.switchTab({ url: url })
				// } else if(type=='back'){
				// 	uni.navigateBack({ delta: 1 })
				// } else {
				// 	uni.navigateTo({ url: url })
				// }
				// this.$emit('Click');
				this.$emit('click');
			}
		},
		computed:{
			Col(){
				if (this.col < 2) return 2;
				else if(this.col > 5) return 5;
				else return this.col;
			},
			Border(){
				if(this.border == 'false' || this.border == 0) return 'noborder';
				else return '';
			},
			
			Order(){ // 排序
				if (this.order < -1) return -1;
				else return this.order;
			}
		}
	}
</script>

<style lang="scss">



	.dot,.badge{
		background-color:#e00;
		border-radius:50px;
		position:absolute;
		z-index:10;
	}
	.dot{
		width:8px;
		height:8px;
		top:12%;
	}
	.badge{
		color:#fff;
		line-height:26rpx;
		font-size:18rpx;
		padding:0 10rpx;
		height:24rpx;
		top:10%;
	}
	
	@for $i from -1 through 25 {
		.order-#{$i}{order:#{$i}}
	}
	@for $x from 2 through 5 {
		
		/* #ifdef MP */
		.grid-item.col-#{$x}{
			width:calc(100% / #{$x});
			float: left;
		}
		/* #endif */
		/* #ifdef APP-PLUS || H5 */
		.grid-item.col-#{$x}{
			flex:0 0 calc(100% / #{$x});
			max-width:calc(100% / #{$x});
		}
		/* #endif */
		.sub{
			.dot{right:calc(50% - 30rpx - 50rpx/#{$x});}
			.badge{right:calc(50% - 40rpx - 90rpx/#{$x});}
		}
	}
	
	.grid-item {
		box-sizing: border-box;
		background: #fff;
		/* #ifndef APP-NVUE */
		display: flex;
		flex-direction:row;
		/* #endif */
		align-items: center;
		justify-content: center;
		position: relative;
		flex-direction: column;
		
		/* #ifdef MP */
		position: relative;
		float: left;
		/* #endif */
	}
	.grid-item{
		background-color:#fff;
		.sub{
			[class*="o-i"]{
				width:80rpx;
				height:80rpx;
				margin:0 auto;
				overflow:hidden;
				display:flex;
				align-items: center;
				&::before{
					display:block;
					width:100%;
					height:100%;
				}
				image{
					display:block;
					width:80%;
					height:80%;
					margin:auto;
				}
			}
		}
		&::before,&::after{
			content:'';
			position:absolute;
			right:0;bottom:0;
			background-color:#f8f8f8;
		}
		&::before{width:2rpx;top:0;}
		&::after{height:2rpx;left:0;}
	}
	.noborder{
		&::before,&::after{
			content:none !important;
		}
	}
	
	.grid-item.col-2{
		padding:36rpx 20rpx;
		[class*="o-i"]{
			margin-bottom:18rpx;
			width:80rpx;height:80rpx;
			&::before{font:normal 64rpx/1.2 'o-icon' !important;}
		}
		.o-text{
			font-size:30rpx;
			line-height:36rpx;
			overflow: hidden;
			text-overflow: ellipsis;
			max-width: 130rpx;
			white-space: nowrap;
		}
		.o-img{
			width:80rpx;
			height:80rpx;
			image{
				width:100%;
				height:100%;
			}
		}
	}
	.grid-item.col-3{
		padding:36rpx 20rpx;
		[class*="o-i"]{
			margin-bottom:14rpx;
			width:72rpx;height:72rpx;
			&::before{font:normal 52rpx/1.2 'o-icon' !important;}
		}
		.o-text{
			font-size:26rpx;
			line-height:32rpx;
			overflow: hidden;
			text-overflow: ellipsis;
			max-width: 130rpx;
			white-space: nowrap;
		}
		.o-img{
			width:72rpx;
			height:72rpx;
			image{
				width:100%;
				height:100%;
			}
		}
	}
	.grid-item.col-4{
		padding:30rpx 20rpx;
		[class*="o-i"]{
			margin-bottom:14rpx;
			width:64rpx;height:64rpx;
			&::before{font:normal 48rpx/1.3 'o-icon' !important;}
		}
		.o-text{
			font-size:24rpx;
			line-height:28rpx;
			overflow: hidden;
			text-overflow: ellipsis;
			max-width: 130rpx;
			white-space: nowrap;
		}
		.o-img{
			width:64rpx;
			height:64rpx;
			image{
				width:100%;
				height:100%;
			}
		}
	}
	.grid-item.col-5{
		padding:20rpx 15rpx;
		[class*="o-i"]{
			margin-bottom:10rpx;
			width:56rpx;height:56rpx;
			&::before{font:normal 42rpx/1.2 'o-icon' !important;}
		}
		.o-text{
			font-size:22rpx;
			line-height:26rpx;
			overflow: hidden;
			text-overflow: ellipsis;
			max-width: 130rpx;
			white-space: nowrap;
		}
		.o-img{
			width:56rpx;
			height:56rpx;
			image{
				width:100%;
				height:100%;
			}
		}
	}
	
	@font-face {
		font-weight: 400;
		font-style: normal;
		font-display:auto;
	    font-family: 'o-icon';
	    src: url('data:application/octet-stream;base64,') format('truetype')
	}

	[class*="o-icon-"] {
	  position:relative;
	  display:block;
	  font:normal 18px/1 "o-icon";
	  font-size:inherit;
	  text-rendering:auto;
	  -webkit-font-smoothing: antialiased;
	  &::before{
		  display:block;
	  }
	}

	/* .o-icon::before{display:inline-block;} */
	.o-icon-add-o:before{content:"\F000";}
	.o-icon-add-square:before{content:"\F001";}
	.o-icon-add:before{content:"\F002";}
	.o-icon-after-sale:before{content:"\F003";}
	.o-icon-aim:before{content:"\F004";}
	.o-icon-alipay:before{content:"\F005";}
	.o-icon-apps-o:before{content:"\F006";}
	.o-icon-arrow-down:before{content:"\F007";}
	.o-icon-arrow-left:before{content:"\F008";}
	.o-icon-arrow-up:before{content:"\F009";}
	.o-icon-arrow:before{content:"\F00A";}
	.o-icon-ascending:before{content:"\F00B";}
	.o-icon-audio:before{content:"\F00C";}
	.o-icon-award-o:before{content:"\F00D";}
	.o-icon-award:before{content:"\F00E";}
	.o-icon-bag-o:before{content:"\F00F";}
	.o-icon-bag:before{content:"\F010";}
	.o-icon-balance-list-o:before{content:"\F011";}
	.o-icon-balance-list:before{content:"\F012";}
	.o-icon-balance-o:before{content:"\F013";}
	.o-icon-balance-pay:before{content:"\F014";}
	.o-icon-bar-chart-o:before{content:"\F015";}
	.o-icon-bars:before{content:"\F016";}
	.o-icon-bell:before{content:"\F017";}
	.o-icon-bill-o:before{content:"\F018";}
	.o-icon-bill:before{content:"\F019";}
	.o-icon-birthday-cake-o:before{content:"\F01A";}
	.o-icon-bookmark-o:before{content:"\F01B";}
	.o-icon-bookmark:before{content:"\F01C";}
	.o-icon-browsing-history-o:before{content:"\F01D";}
	.o-icon-browsing-history:before{content:"\F01E";}
	.o-icon-brush-o:before{content:"\F01F";}
	.o-icon-bulb-o:before{content:"\F020";}
	.o-icon-bullhorn-o:before{content:"\F021";}
	.o-icon-calender-o:before{content:"\F022";}
	.o-icon-card:before{content:"\F023";}
	.o-icon-cart-circle-o:before{content:"\F024";}
	.o-icon-cart-circle:before{content:"\F025";}
	.o-icon-cart-o:before{content:"\F026";}
	.o-icon-cart:before{content:"\F027";}
	.o-icon-cash-back-record:before{content:"\F028";}
	.o-icon-cash-on-deliver:before{content:"\F029";}
	.o-icon-cashier-o:before{content:"\F02A";}
	.o-icon-certificate:before{content:"\F02B";}
	.o-icon-chart-trending-o:before{content:"\F02C";}
	.o-icon-chat-o:before{content:"\F02D";}
	.o-icon-chat:before{content:"\F02E";}
	.o-icon-checked:before{content:"\F02F";}
	.o-icon-circle:before{content:"\F030";}
	.o-icon-clear:before{content:"\F031";}
	.o-icon-clock-o:before{content:"\F032";}
	.o-icon-clock:before{content:"\F033";}
	.o-icon-close:before{content:"\F034";}
	.o-icon-closed-eye:before{content:"\F035";}
	.o-icon-cluster-o:before{content:"\F036";}
	.o-icon-cluster:before{content:"\F037";}
	.o-icon-column:before{content:"\F038";}
	.o-icon-comment-circle-o:before{content:"\F039";}
	.o-icon-comment-circle:before{content:"\F03A";}
	.o-icon-comment-o:before{content:"\F03B";}
	.o-icon-comment:before{content:"\F03C";}
	.o-icon-completed:before{content:"\F03D";}
	.o-icon-contact:before{content:"\F03E";}
	.o-icon-coupon-o:before{content:"\F03F";}
	.o-icon-coupon:before{content:"\F040";}
	.o-icon-credit-pay:before{content:"\F041";}
	.o-icon-cross:before{content:"\F042";}
	.o-icon-debit-pay:before{content:"\F043";}
	.o-icon-delete:before{content:"\F044";}
	.o-icon-descending:before{content:"\F045";}
	.o-icon-description:before{content:"\F046";}
	.o-icon-desktop-o:before{content:"\F047";}
	.o-icon-diamond-o:before{content:"\F048";}
	.o-icon-diamond:before{content:"\F049";}
	.o-icon-discount:before{content:"\F04A";}
	.o-icon-down:before{content:"\F04B";}
	.o-icon-ecard-pay:before{content:"\F04C";}
	.o-icon-edit:before{content:"\F04D";}
	.o-icon-ellipsis:before{content:"\F04E";}
	.o-icon-empty:before{content:"\F04F";}
	.o-icon-envelop-o:before{content:"\F050";}
	.o-icon-exchange:before{content:"\F051";}
	.o-icon-expand-o:before{content:"\F052";}
	.o-icon-expand:before{content:"\F053";}
	.o-icon-eye-o:before{content:"\F054";}
	.o-icon-eye:before{content:"\F055";}
	.o-icon-fail:before{content:"\F056";}
	.o-icon-failure:before{content:"\F057";}
	.o-icon-filter-o:before{content:"\F058";}
	.o-icon-fire-o:before{content:"\F059";}
	.o-icon-fire:before{content:"\F05A";}
	.o-icon-flag-o:before{content:"\F05B";}
	.o-icon-flower-o:before{content:"\F05C";}
	.o-icon-free-postage:before{content:"\F05D";}
	.o-icon-friends-o:before{content:"\F05E";}
	.o-icon-friends:before{content:"\F05F";}
	.o-icon-gem-o:before{content:"\F060";}
	.o-icon-gem:before{content:"\F061";}
	.o-icon-gift-card-o:before{content:"\F062";}
	.o-icon-gift-card:before{content:"\F063";}
	.o-icon-gift-o:before{content:"\F064";}
	.o-icon-gift:before{content:"\F065";}
	.o-icon-gold-coin-o:before{content:"\F066";}
	.o-icon-gold-coin:before{content:"\F067";}
	.o-icon-good-job-o:before{content:"\F068";}
	.o-icon-good-job:before{content:"\F069";}
	.o-icon-goods-collect-o:before{content:"\F06A";}
	.o-icon-goods-collect:before{content:"\F06B";}
	.o-icon-graphic:before{content:"\F06C";}
	.o-icon-home-o:before{content:"\F06D";}
	.o-icon-hot-o:before{content:"\F06E";}
	.o-icon-hot-sale-o:before{content:"\F06F";}
	.o-icon-hot-sale:before{content:"\F070";}
	.o-icon-hot:before{content:"\F071";}
	.o-icon-hotel-o:before{content:"\F072";}
	.o-icon-idcard:before{content:"\F073";}
	.o-icon-info-o:before{content:"\F074";}
	.o-icon-info:before{content:"\F075";}
	.o-icon-invition:before{content:"\F076";}
	.o-icon-label-o:before{content:"\F077";}
	.o-icon-label:before{content:"\F078";}
	.o-icon-like-o:before{content:"\F079";}
	.o-icon-like:before{content:"\F07A";}
	.o-icon-live:before{content:"\F07B";}
	.o-icon-location-o:before{content:"\F07C";}
	.o-icon-location:before{content:"\F07D";}
	.o-icon-lock:before{content:"\F07E";}
	.o-icon-logistics:before{content:"\F07F";}
	.o-icon-manager-o:before{content:"\F080";}
	.o-icon-manager:before{content:"\F081";}
	.o-icon-map-marked:before{content:"\F082";}
	.o-icon-medel-o:before{content:"\F083";}
	.o-icon-medel:before{content:"\F084";}
	.o-icon-more-o:before{content:"\F085";}
	.o-icon-more:before{content:"\F086";}
	.o-icon-music-o:before{content:"\F087";}
	.o-icon-music:before{content:"\F088";}
	.o-icon-new-arrival-o:before{content:"\F089";}
	.o-icon-new-arrival:before{content:"\F08A";}
	.o-icon-new-o:before{content:"\F08B";}
	.o-icon-new:before{content:"\F08C";}
	.o-icon-newspaper-o:before{content:"\F08D";}
	.o-icon-notes-o:before{content:"\F08E";}
	.o-icon-orders-o:before{content:"\F08F";}
	.o-icon-other-pay:before{content:"\F090";}
	.o-icon-paid:before{content:"\F091";}
	.o-icon-passed:before{content:"\F092";}
	.o-icon-pause-circle-o:before{content:"\F093";}
	.o-icon-pause-circle:before{content:"\F094";}
	.o-icon-pause:before{content:"\F095";}
	.o-icon-peer-pay:before{content:"\F096";}
	.o-icon-pending-payment:before{content:"\F097";}
	.o-icon-phone-circle-o:before{content:"\F098";}
	.o-icon-phone-circle:before{content:"\F099";}
	.o-icon-phone-o:before{content:"\F09A";}
	.o-icon-phone:before{content:"\F09B";}
	.o-icon-photo-o:before{content:"\F09C";}
	.o-icon-photo:before{content:"\F09D";}
	.o-icon-photograph:before{content:"\F09E";}
	.o-icon-play-circle-o:before{content:"\F09F";}
	.o-icon-play-circle:before{content:"\F0A0";}
	.o-icon-play:before{content:"\F0A1";}
	.o-icon-plus:before{content:"\F0A2";}
	.o-icon-point-gift-o:before{content:"\F0A3";}
	.o-icon-point-gift:before{content:"\F0A4";}
	.o-icon-points:before{content:"\F0A5";}
	.o-icon-printer:before{content:"\F0A6";}
	.o-icon-qr-invalid:before{content:"\F0A7";}
	.o-icon-qr:before{content:"\F0A8";}
	.o-icon-question-o:before{content:"\F0A9";}
	.o-icon-question:before{content:"\F0AA";}
	.o-icon-records:before{content:"\F0AB";}
	.o-icon-refund-o:before{content:"\F0AC";}
	.o-icon-replay:before{content:"\F0AD";}
	.o-icon-scan:before{content:"\F0AE";}
	.o-icon-search:before{content:"\F0AF";}
	.o-icon-send-gift-o:before{content:"\F0B0";}
	.o-icon-send-gift:before{content:"\F0B1";}
	.o-icon-service-o:before{content:"\F0B2";}
	.o-icon-service:before{content:"\F0B3";}
	.o-icon-setting-o:before{content:"\F0B4";}
	.o-icon-setting:before{content:"\F0B5";}
	.o-icon-share:before{content:"\F0B6";}
	.o-icon-shop-collect-o:before{content:"\F0B7";}
	.o-icon-shop-collect:before{content:"\F0B8";}
	.o-icon-shop-o:before{content:"\F0B9";}
	.o-icon-shop:before{content:"\F0BA";}
	.o-icon-shopping-cart-o:before{content:"\F0BB";}
	.o-icon-shopping-cart:before{content:"\F0BC";}
	.o-icon-shrink:before{content:"\F0BD";}
	.o-icon-sign:before{content:"\F0BE";}
	.o-icon-smile-comment-o:before{content:"\F0BF";}
	.o-icon-smile-comment:before{content:"\F0C0";}
	.o-icon-smile-o:before{content:"\F0C1";}
	.o-icon-smile:before{content:"\F0C2";}
	.o-icon-star-o:before{content:"\F0C3";}
	.o-icon-star:before{content:"\F0C4";}
	.o-icon-stop-circle-o:before{content:"\F0C5";}
	.o-icon-stop-circle:before{content:"\F0C6";}
	.o-icon-stop:before{content:"\F0C7";}
	.o-icon-success:before{content:"\F0C8";}
	.o-icon-thumb-circle-o:before{content:"\F0C9";}
	.o-icon-thumb-circle:before{content:"\F0CA";}
	.o-icon-todo-list-o:before{content:"\F0CB";}
	.o-icon-todo-list:before{content:"\F0CC";}
	.o-icon-tosend:before{content:"\F0CD";}
	.o-icon-tv-o:before{content:"\F0CE";}
	.o-icon-umbrella-circle:before{content:"\F0CF";}
	.o-icon-underway-o:before{content:"\F0D0";}
	.o-icon-underway:before{content:"\F0D1";}
	.o-icon-upgrade:before{content:"\F0D2";}
	.o-icon-user-circle-o:before{content:"\F0D3";}
	.o-icon-user-o:before{content:"\F0D4";}
	.o-icon-video-o:before{content:"\F0D5";}
	.o-icon-video:before{content:"\F0D6";}
	.o-icon-vip-card-o:before{content:"\F0D7";}
	.o-icon-vip-card:before{content:"\F0D8";}
	.o-icon-volume-o:before{content:"\F0D9";}
	.o-icon-volume:before{content:"\F0DA";}
	.o-icon-wap-home-o:before{content:"\F0DB";}
	.o-icon-wap-home:before{content:"\F0DC";}
	.o-icon-wap-nav:before{content:"\F0DD";}
	.o-icon-warn-o:before{content:"\F0DE";}
	.o-icon-warning-o:before{content:"\F0DF";}
	.o-icon-warning:before{content:"\F0E0";}
	.o-icon-weapp-nav:before{content:"\F0E1";}
	.o-icon-wechat:before{content:"\F0E2";}
	.o-icon-youzan-shield:before{content:"\F0E3";}

</style>
